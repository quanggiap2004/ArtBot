...
```json
query{  devices (query : {}) {    page{      edges{        cursor,        node{          _id,          deviceName,          UUID,          pairingCode,          currentType,          currentAssetId,          currentPlaylistId,          localAppVersion,        }      }    }  }}
```
...
```json
query{  assets (query : {originalFileName:"Houston Weather Test"}) {    page{      edges{        cursor,        node{          _id,          appType,          fileType,          name,          filename        }      }    }  }}
```
